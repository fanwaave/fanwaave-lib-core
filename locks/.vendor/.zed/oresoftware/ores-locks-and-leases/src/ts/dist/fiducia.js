...
export function cleartextRefusal(baseUrl, hasCredential, allow) {
    if (!hasCredential || allow || !baseUrl.startsWith("http://"))
        return undefined;
...
export function generatedHolder() {
...
function encodeWireInteger(value) {
    if (value < 0n || value > BigInt(Number.MAX_SAFE_INTEGER)) {
        throw new RangeError(`fiducia: fencing token ${value} cannot be represented exactly by the current numeric JSON wire format`);
    }
    return Number(value);
}
export class FiduciaLease {
    #base;
    #headers;
    #fetch;
    #refusal;
    ...
    async #post(path, body) {
        ...
        if (response.status >= 300)
            throw new Error(`fiducia: HTTP ${response.status}: ${text.trim()}`);
        if (!text)
            return {};
        ...
        for (;;) {
            ...
            if (out["acquired"] === true) {
                ...
                if (fencingToken === undefined)
                    throw LockError.transport(key, new Error("fiducia: acquired without a fencing token"));
