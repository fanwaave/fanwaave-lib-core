...
-spec be_ok({ok, HMO} | {error, any()}) -> HMO.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, HMT}) -> HMT.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(HMW)) -> HMW.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
