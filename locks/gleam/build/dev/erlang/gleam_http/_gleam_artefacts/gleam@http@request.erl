...
-spec set_header(request(EHF), binary(), binary()) -> request(EHF).
...
-spec prepend_header(request(EHI), binary(), binary()) -> request(EHI).
...
-spec set_body(request(any()), EHN) -> request(EHN).
set_body(Req, Body) ->
    {request,
...
-spec map(request(EHP), fun((EHP) -> EHR)) -> request(EHR).
...
-file("src/gleam/http/request.gleam", 175).
?DOC(" Set the method of the request.\n").
-spec set_method(request(EIF), gleam@http:method()) -> request(EIF).
...
-file("src/gleam/http/request.gleam", 182).
?DOC(
...
-file("src/gleam/http/request.gleam", 197).
...
-file("src/gleam/http/request.gleam", 205).
?DOC(" Set the scheme (protocol) of the request.\n").
-spec set_scheme(request(EIM), gleam@http:scheme()) -> request(EIM).
...
-file("src/gleam/http/request.gleam", 211).
?DOC(" Set the host of the request.\n").
-spec set_host(request(EIP), binary()) -> request(EIP).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-file("src/gleam/http/request.gleam", 217).
?DOC(" Set the port of the request.\n").
-spec set_port(request(EIS), integer()) -> request(EIS).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-file("src/gleam/http/request.gleam", 223).
?DOC(" Set the path of the request.\n").
-spec set_path(request(EIV), binary()) -> request(EIV).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-file("src/gleam/http/request.gleam", 234).
?DOC(
...
-spec set_cookie(request(EIY), binary(), binary()) -> request(EIY).
...
-file("src/gleam/http/request.gleam", 257).
?DOC(
...
-file("src/gleam/http/request.gleam", 272).
?DOC(
...
-spec remove_cookie(request(EJE), binary()) -> request(EJE).
