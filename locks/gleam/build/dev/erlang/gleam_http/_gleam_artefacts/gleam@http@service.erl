...
-spec map_response_body(
    fun((EUV) -> gleam@http@response:response(EUW)),
    fun((EUW) -> EUY)
) -> fun((EUV) -> gleam@http@response:response(EUY)).
...
-spec prepend_response_header(
    fun((EVA) -> gleam@http@response:response(EVB)),
    binary(),
    binary()
) -> fun((EVA) -> gleam@http@response:response(EVB)).
...
-spec ensure_post(gleam@http@request:request(EVE)) -> {ok,
        gleam@http@request:request(EVE)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(EVN)) -> EVP)) -> fun((gleam@http@request:request(EVN)) -> EVP).
