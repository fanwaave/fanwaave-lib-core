...
-type response(EQZ) :: {response, integer(), list({binary(), binary()}), EQZ}.

-file("src/gleam/http/response.gleam", 89).
?DOC(" Set the body of the response, overwriting any existing body.\n").
-spec set_body(response(any()), ERW) -> response(ERW).
set_body(Response, Body) ->
    {response, erlang:element(2, Response), erlang:element(3, Response), Body}.

-file("src/gleam/http/response.gleam", 26).
?DOC(
...
-spec try_map(response(ERA), fun((ERA) -> {ok, ERC} | {error, ERD})) -> {ok,
        response(ERC)} |
    {error, ERD}.
...
-file("src/gleam/http/response.gleam", 39).
?DOC(
...
-file("src/gleam/http/response.gleam", 47).
?DOC(
...
-file("src/gleam/http/response.gleam", 61).
?DOC(
...
-spec set_header(response(ERO), binary(), binary()) -> response(ERO).
...
-file("src/gleam/http/response.gleam", 78).
?DOC(
...
-spec prepend_header(response(ERR), binary(), binary()) -> response(ERR).
...
-file("src/gleam/http/response.gleam", 121).
?DOC(
...
-file("src/gleam/http/response.gleam", 134).
...
-file("src/gleam/http/response.gleam", 150).
?DOC(
    " Expire a cookie value for a client\n"
    "\n"
    " Note: The attributes value should be the same as when the response cookie was set.\n"
).
-spec expire_cookie(response(ESJ), binary(), gleam@http@cookie:attributes()) -> response(ESJ).
