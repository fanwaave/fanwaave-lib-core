...
-type transaction_error(HOQ) :: {transaction_query_error, query_error()} |
    {transaction_rolled_back, HOQ}.

-type returned(HOR) :: {returned, integer(), list(HOR)}.
...
-opaque 'query'(HOS) :: {'query',
        binary(),
        list(value()),
        gleam@dynamic@decode:decoder(HOS),
        integer()}.
...
-spec array(fun((HPJ) -> value()), list(HPJ)) -> value().
...
-spec transaction_layer(
    single_connection(),
    fun((connection()) -> {ok, HPU} | {error, HPV})
) -> {ok, HPU} | {error, transaction_error(HPV)}.
...
-spec transaction(connection(), fun((connection()) -> {ok, HPN} | {error, HPO})) -> {ok,
        HPN} |
    {error, transaction_error(HPO)}.
...
-spec nullable(fun((HQE) -> value()), gleam@option:option(HQE)) -> value().
...
-spec returning('query'(any()), gleam@dynamic@decode:decoder(HQQ)) -> 'query'(HQQ).
...
-spec parameter('query'(HQT), value()) -> 'query'(HQT).
...
-spec timeout('query'(HQW), integer()) -> 'query'(HQW).
...
-spec execute('query'(HQZ), connection()) -> {ok, returned(HQZ)} |
    {error, query_error()}.
execute(Query, Pool) ->
