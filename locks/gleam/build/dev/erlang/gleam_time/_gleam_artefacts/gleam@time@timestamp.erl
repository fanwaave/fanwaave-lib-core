...
to_http_date(Timestamp) ->
    ...
    Out@1 = <<<<<<<<<<Out/binary, (N2(erlang:element(4, Date)))/binary>>/binary,
                    " "/utf8>>/binary,
                (month_to_http_string(erlang:element(3, Date)))/binary>>/binary,
            " "/utf8>>/binary,
        (N4(erlang:element(2, Date)))/binary>>,
    Out@2 = <<Out@1/binary, " "/utf8>>,
    Out@3 = <<<<<<<<<<Out@2/binary, (N2(erlang:element(2, Time)))/binary>>/binary,
                    ":"/utf8>>/binary,
                (N2(erlang:element(3, Time)))/binary>>/binary,
            ":"/utf8>>/binary,
        (N2(erlang:element(4, Time)))/binary>>,
...
-spec parse_time_without_nanoseconds(bitstring()) -> {ok,
        {gleam@time@calendar:time_of_day(), bitstring()}} |
    {error, nil}.
parse_time_without_nanoseconds(Bytes) ->
    gleam@result:'try'(
        ...
        fun(_use0) ->
            {Hours, Bytes@1} = _use0,
            gleam@result:'try'(
                accept_byte(Bytes@1, 16#3A),
                fun(Bytes@2) ->
                    gleam@result:'try'(
                        parse_minutes(Bytes@2),
                        fun(_use0@1) ->
                            {Minutes, Bytes@3} = _use0@1,
                            gleam@result:'try'(
                                accept_byte(Bytes@3, 16#3A),
                                fun(Bytes@4) ->
                                    gleam@result:'try'(
                                        parse_seconds(Bytes@4),
                                        fun(_use0@2) ->
...
-spec parse_asctime_day(bitstring()) -> {ok, {integer(), bitstring()}} |
    {error, nil}.
parse_asctime_day(Bytes) ->
    case Bytes of
        <<Byte, Bytes@1/binary>> when Byte =:= 16#20 ->
...
-spec parse_http_month(bitstring()) -> {ok,
        {gleam@time@calendar:month(), bitstring()}} |
    {error, nil}.
...
parse_asctime_date(Input) ->
    ...
    gleam@result:'try'(
        ...
        fun(_use0) ->
            {Month, Bytes@1} = _use0,
            gleam@result:'try'(
                accept_byte(Bytes@1, 16#20),
                fun(Bytes@2) ->
                    gleam@result:'try'(
                        parse_asctime_day(Bytes@2),
                        fun(_use0@1) ->
                            {Day, Bytes@3} = _use0@1,
                            gleam@result:'try'(
                                accept_byte(Bytes@3, 16#20),
                                fun(Bytes@4) ->
                                    gleam@result:'try'(
                                        parse_time_without_nanoseconds(Bytes@4),
                                        fun(_use0@2) ->
                                            {Time, Bytes@5} = _use0@2,
                                            gleam@result:'try'(
                                                accept_byte(Bytes@5, 16#20),
                                                fun(Bytes@6) ->
                                                    gleam@result:'try'(
                                                        parse_digits(Bytes@6, 4),
                                                        fun(_use0@3) ->
                                    ...
                                    gleam@result:'try'(
                                        ...
                                        fun(_use0@2) ->
                                            ...
                                            gleam@result:'try'(
                                                ...
                                                fun(Bytes@6) ->
                                                    gleam@result:'try'(
                                                        parse_time_without_nanoseconds(
                                                            Bytes@6
                                                        ),
                                                        fun(_use0@3) ->
