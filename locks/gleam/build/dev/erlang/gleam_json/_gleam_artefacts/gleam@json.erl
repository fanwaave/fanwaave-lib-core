...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(FBY)) -> {ok, FBY} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(FBS)) -> {ok, FBS} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(FBO)) -> {ok, FBO} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(FCE), fun((FCE) -> json())) -> json().
...
-spec array(list(FCI), fun((FCI) -> json())) -> json().
