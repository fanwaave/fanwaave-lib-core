...
-type message(FEG) :: {message, FEG} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(FEH, FEI) :: {continue,
        FEH,
        gleam@option:option(gleam@erlang@process:selector(FEI))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(FEJ, FEK) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        FEJ,
        gleam@erlang@process:selector(message(FEK)),
        gleam@otp@system:debug_state(),
        fun((FEJ, FEK) -> next(FEJ, FEK))}.

-type started(FEL) :: {started, gleam@erlang@process:pid_(), FEL}.

-opaque initialised(FEM, FEN, FEO) :: {initialised,
        FEM,
        gleam@option:option(gleam@erlang@process:selector(FEN)),
        FEO}.

-opaque builder(FEP, FEQ, FER) :: {builder,
        fun((gleam@erlang@process:subject(FEQ)) -> {ok,
                initialised(FEP, FEQ, FER)} |
            {error, binary()}),
        integer(),
        fun((FEP, FEQ) -> next(FEP, FEQ)),
        gleam@option:option(gleam@erlang@process:name(FEQ))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(FES) :: {ack, {ok, FES} | {error, binary()}} |
...
-spec continue(FEX) -> next(FEX, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(FFJ, FFK), gleam@erlang@process:selector(FFK)) -> next(FFJ, FFK).
...
-spec initialised(FFQ) -> initialised(FFQ, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(FFV, any(), FFX),
    gleam@erlang@process:selector(FGB)
) -> initialised(FFV, FGB, FFX).
...
-spec returning(initialised(FGG, FGH, any()), FGM) -> initialised(FGG, FGH, FGM).
...
-spec new(FGQ) -> builder(FGQ, FGR, gleam@erlang@process:subject(FGR)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(FGW)) -> {ok, initialised(FGY, FGW, FGZ)} |
        {error, binary()})
) -> builder(FGY, FGW, FGZ).
...
-spec on_message(builder(FHI, FHJ, FHK), fun((FHI, FHJ) -> next(FHI, FHJ))) -> builder(FHI, FHJ, FHK).
...
-spec named(builder(FHT, FHU, FHV), gleam@erlang@process:name(FHU)) -> builder(FHT, FHU, FHV).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(FII))) -> gleam@erlang@process:selector(message(FII)).
...
-spec receive_message(self(any(), FIE)) -> message(FIE).
...
-spec initialise_actor(
    builder(any(), any(), FJB),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, FJB} | {error, binary()})
...
-spec start(builder(any(), any(), FJO)) -> {ok, started(FJO)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(FJV), FJV) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(FJX),
    integer(),
    fun((gleam@erlang@process:subject(FJZ)) -> FJX)
) -> FJZ.
