...
-type child_specification(FXL) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(FXL)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(FXM)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(FXM).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(FXR)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(FXR).
supervisor(Start) ->
...
-spec significant(child_specification(FXW), boolean()) -> child_specification(FXW).
...
-spec timeout(child_specification(FXZ), integer()) -> child_specification(FXZ).
timeout(Child, Ms) ->
...
-spec restart(child_specification(FYC), restart()) -> child_specification(FYC).
...
-spec map_data(child_specification(FYF), fun((FYF) -> FYH)) -> child_specification(FYH).
