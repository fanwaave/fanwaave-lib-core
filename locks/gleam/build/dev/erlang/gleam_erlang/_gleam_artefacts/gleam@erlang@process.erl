...
-opaque subject(DLD) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DLD)}.

-type name(DLE) :: any() | {gleam_phantom, DLE}.

-type do_not_leak() :: any().

-type selector(DLF) :: any() | {gleam_phantom, DLF}.
...
-spec 'receive'(subject(DMD), integer()) -> {ok, DMD} | {error, nil}.
...
-spec receive_forever(subject(DML)) -> DML.
...
-spec selector_receive(selector(DMP), integer()) -> {ok, DMP} | {error, nil}.
...
-spec selector_receive_forever(selector(DMT)) -> DMT.
...
-spec map_selector(selector(DMV), fun((DMV) -> DMX)) -> selector(DMX).
map_selector(A, B) ->
...
-spec merge_selector(selector(DMZ), selector(DMZ)) -> selector(DMZ).
merge_selector(A, B) ->
...
-spec select_trapped_exits(selector(DND), fun((exit_message()) -> DND)) -> selector(DND).
...
-spec select_map(selector(DNK), subject(DNM), fun((DNM) -> DNK)) -> selector(DNK).
...
-spec select(selector(DNG), subject(DNG)) -> selector(DNG).
...
-spec deselect(selector(DNP), subject(any())) -> selector(DNP).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DNU)
) -> selector(DNU).
...
-spec select_other(selector(DNY), fun((gleam@dynamic:dynamic_()) -> DNY)) -> selector(DNY).
...
-spec select_specific_monitor(selector(DOK), monitor(), fun((down()) -> DOK)) -> selector(DOK).
...
-spec select_monitors(selector(DON), fun((down()) -> DON)) -> selector(DON).
...
-spec deselect_specific_monitor(selector(DOQ), monitor()) -> selector(DOQ).
...
-spec perform_call(
    subject(DOT),
    fun((subject(DOV)) -> DOT),
    fun((selector(DOV)) -> {ok, DOV} | {error, nil})
) -> DOV.
...
-spec call(subject(DPA), integer(), fun((subject(DPC)) -> DPA)) -> DPC.
...
-spec call_forever(subject(DPE), fun((subject(DPG)) -> DPE)) -> DPG.
...
-spec send_after(subject(DPM), integer(), DPM) -> timer().
