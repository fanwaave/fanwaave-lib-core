...
-spec map({ok, CMW} | {error, CMX}, fun((CMW) -> CNA)) -> {ok, CNA} |
    {error, CMX}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CND} | {error, CNE}, fun((CNE) -> CNH)) -> {ok, CND} |
    {error, CNH}.
...
-spec flatten({ok, {ok, CNK} | {error, CNL}} | {error, CNL}) -> {ok, CNK} |
    {error, CNL}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, CNS} | {error, CNT}, fun((CNS) -> {ok, CNW} | {error, CNT})) -> {ok,
        CNW} |
    {error, CNT}.
'try'(Result, Fun) ->
    case Result of
...
-spec 'or'({ok, CON} | {error, COO}, {ok, CON} | {error, COO}) -> {ok, CON} |
    {error, COO}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, COV} | {error, COW}, fun(() -> {ok, COV} | {error, COW})) -> {ok,
        COV} |
    {error, COW}.
...
-spec partition_loop(list({ok, CPS} | {error, CPT}), list(CPS), list(CPT)) -> {list(CPS),
    list(CPT)}.
