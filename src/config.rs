...
impl CoreConfig {
    pub fn from_env() -> Result<Self, CoreError> {
        let database_url = std::env::var("FANWAAVE_DATABASE_URL").map_err(|_| CoreError::InvalidDatabaseUrl)?;
        ...
        Ok(Self {
            database_url,
            flavor,
            read_only: std::env::var("FANWAAVE_DB_READ_ONLY").ok().as_deref() != Some("0"),
        })
    }
}
